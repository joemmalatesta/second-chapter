			...
			try {
				...
				const filterBooks = (booksData: any[]) => {
					return booksData.filter(
						(book: any) => !book.checkedOutTo && (selectedGenres.length === 0 || selectedGenres.includes(book.genre.toLowerCase))
					);
				};
				...
				if (isLocationAvailable) {
					navigator.geolocation.getCurrentPosition(
						(position) => {
							...
							const sortedBooks = filteredBooks.sort((a: any, b: any) => a.distance - b.distance);
						...
						<ul className="flex flex-wrap gap-1.5 genre-selections">
							{genres.map((genre, index) => (
								<li key={index} className="genre-item">
									...
									<label
										htmlFor={`genre${index}`}
										className={`genre-btn inline-block px-4 py-2 rounded-full border cursor-pointer transition-colors duration-200 ease-in-out ${
											selectedGenres.includes(genre) ? "genre-btn-active" : ""
										}`}
									>
										{genre}
									</label>
				...
				{books.map((book) => (
					<article className="book-item w-52 rounded-lg flex flex-col justify-between items-center">
						<div className="book-div rounded-lg overflow-hidden w-44">
							<img src={book.image} alt="Book cover" className=" book-cover rounded-lg overflow-hidden scale-[102%] max-h-44 object-fit" />
						</div>
						<div className=" w-full flex flex-col">
							<div className="book-info">
								<h3 className="book-title overflow-ellipsis">{book.title}</h3>
								<p className=" text-white text-sm opacity-60 overflow-ellipsis">{book.author}</p>
								<div className="w-full flex justify-between items-center">
									<div className=" flex gap-1 items-center text-sm">
										...
										<span>
											{book.distance >= 0.1 ? book.distance : ".1"}
											mi
										</span>
									</div>
									<button onClick={() => claimBook(book.isbn)} className="bg-[#6c584c]/80 hover:bg-[#6c584c] p-2 rounded-lg ">
										Claim
									</button>
								</div>
							</div>
